/**
 * http://usejsdoc.org/
 */
var audio_context;
var recorder;
var intervalKey;

/*
var url = 'ws://' + document.domain + ':9001' 

if( window.location.protocol == "http:"){
	url = 'ws://' + document.domain + ':9001' 
}else{
	url = 'wss://' + document.domain + ':9002' 
}
*/
var url = 'ws://' + document.domain + ':' +window.location.port+"/audio";
if( window.location.protocol == "http:"){
	url = 'ws://' + document.domain + ':' +window.location.port+"/audio";
}else{
	url = 'wss://' + document.domain + ':' +window.location.port+"/audio";
}


var reco = new Vue({
	el : '#record',
	data : {
		returnResult : 'No result',
		isRecord:false,
		isRecognize:false
	},
	computed: {
		  recordbuttonSwitch: function () {
		    if(!this.isRecord){
		    	return {style:"btn btn-primary",value:"Record"};
		    }else{
		    	return {style:"btn btn-danger", value:"Stop"};
		    }
		  },
		  recobuttonSwitch: function () {
			    if(!this.isRecognize){
			    	return {style:"btn btn-primary",value:"Send"};
			    }else{
			    	return {style:"btn btn-warning", value:"Running recognization..."};
			    }
		}
	},
	methods : {
		audiorecord : function() {
			recorder && recorder.record();
			
			if(!this.isRecord){
	            ws.send("start");
	            intervalKey = setInterval(function() {
	            	recorder.exportWAV(function(blob) {
	                	
	                    ws.send(blob);
	                    recorder.clear();
	                });
	            }, 100);
	            this.isRecord=true;
			}else{
				this.isRecord=false;
				recorder && recorder.stop();
				clearInterval(intervalKey);
				ws.send("stop");
				recorder.clear();
				ws.send("analyze");
				this.isRecognize=true;
			}
            
		},
		exportFile:function(){
			
			ws.send("@recognize@: " + this.returnResult);
			
		}
	}
});

var ws = new WebSocket(url);
ws.onopen = function () {
	__log("Openened connection to websocket");
};

//Receive message from server
ws.onmessage = function(e) {
    //var jsonResponse = jQuery.parseJSON(e.data );
    __log(e.data);
    
    if (e.data.indexOf("@action@: ") != -1) {
    	
    	//{"description": "I want to pay credit card", "action": [["@money transfer", 0.0], ["@credit card", 0.4], ["@investment", 0.8333333333333334], ["@foreign exchange", 0.8333333333333334]]}
    	var responseObj = JSON.parse(e.data.substring(9));
    	var action = responseObj.action;
    	
    	if (action && action[0] && action[0][0]) {
    		var page = action[0][0];
    		if (page === "@money transfer") {
    			window.location = "./moneytransfer.html"
    		} else if (page === "@credit card") {
    			window.location = "./creditCard.html"
    		} else if (page === "@investment") {
    			window.locatoin = "./investment.html"
    		} else if (page === "@foreign exchange") {
    			window.location = "./forex.html";
    		} else {
    			alert("can not found any action");
    		}
    	}
    	
    	
    } else {
    	reco.returnResult=e.data;
    }
    
    reco.isRecognize=false;
    //if (jsonResponse.hypotheses.length > 0) {
    //   var bestMatch = jsonResponse.hypotheses[0].utterance;
        //$("#outputText").text(bestMatch);
    //}
}

function __log(e, data) {
    console.log( "\n" + e + " " + (data || '') );
  }

function startUserMedia(stream) {
    var input = audio_context.createMediaStreamSource(stream);
    console.log('Media stream created.');

    // Uncomment if you want the audio to feedback directly
    //input.connect(audio_context.destination);
    //__log('Input connected to audio context destination.');
    
    recorder = new Recorder(input);
    __log('Recorder initialised.');
  }
window.onload = function init() {
    try {
      // webkit shim
      window.AudioContext = window.AudioContext || window.webkitAudioContext;
      navigator.getUserMedia = navigator.getUserMedia || navigator.webkitGetUserMedia;
      window.URL = window.URL || window.webkitURL;
      
      audio_context = new AudioContext;
      __log ('Sample Rate:'+audio_context.sampleRate)
      __log('Audio context set up.');
      __log('navigator.getUserMedia ' + (navigator.getUserMedia ? 'available.' : 'not present!'));
    } catch (e) {
      alert('No web audio support in this browser!');
    }
    
    navigator.getUserMedia({audio: true}, startUserMedia, function(e) {
      __log('No live audio input: ' + e);
    });
  };
  